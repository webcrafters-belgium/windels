...
/** Haalt een numerieke waarde uit Onfact "currency object" ({ "_": 123, "EUR": 123 }) of scalar. */
function moneyValue($v, ?string $currency = null): ?float {
    if ($v === null) return null;
    if (is_numeric($v)) return (float)$v;
    if (is_array($v)) {
        // 1) voorkeursvolgorde: "_" -> specifieke currency -> eerste numerieke
...
/** Formatteer bedrag (centen -> euro indien nodig) */
function moneyFormat(?float $num): string {
    if ($num === null) return '—';
    // Als het op centen lijkt (heel groot integer), deel door 100
    if ($num > 99999 && fmod($num, 1.0) === 0.0) $num = $num / 100.0;
    return number_format($num, 2, ',', '.');
}

/** Zoek totaal incl. op gekende paden, anders som uit lijnen (invoice_lines[].total_amount_incl). */
function extractTotalIncl(array $inv): ?float {
    $currency = (string) (pick($inv, ['currency_id']) ?? 'EUR');

    // Directe total-velden (meest typisch voor Onfact)
    $candidates = [
            'total_amount_incl',
    ...
    if (is_array($lines) && $lines) {
        ...
        foreach ($lines as $ln) {
            ...
            // 3) fallback: total_price_incl / base_total_amount_incl etc.
...
/** Vraag detail op en haal daar het totaal (incl). */
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $email !== '') {
    ...
    // Facturen + defensieve filter + detailtotaal
    if ($contact && !empty($contact['id'])) {
        ...
        // Verrijken met totaal via detail endpoint

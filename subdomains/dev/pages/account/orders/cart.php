...
if(isset($mysqli) && $mysqli instanceof mysqli){
  ...
  try{
    ...
    // Tabel bestaat nog niet of andere DB-fout -> laat $percentageuitvaart op 1
    // eventueel loggen als je wil
  }
}
// CSRF
...
/* -- Failsafe / Dev helper: zorg dat tabellen ok zijn (geen UNIQUE merges) -- */
...
function ensure_cart_tables(mysqli $db): void {
  ...
  if(!$hasItems){
    // ✅ geen UNIQUE keys die samenvoegen
    ...
    // 🚧 bestaande install: verwijder UNIQUE indexen die samenvoegen
    if (index_exists($db,'cart_items','uniq_item')) {
      $db->query("ALTER TABLE `cart_items` DROP INDEX `uniq_item`");
    }
    if (index_exists($db,'cart_items','uniq_cart_item_variant')) {
      $db->query("ALTER TABLE `cart_items` DROP INDEX `uniq_cart_item_variant`");
    }
    // Zorg dat variant_kolommen bestaan (non-breaking)
    if (!col_exists($db,'cart_items','variant_meta')) {
      // JSON niet overal beschikbaar -> fallback TEXT
    ...
    if (!col_exists($db,'cart_items','variant_key')) {
      ...
      // normaliseer NOT NULL + default
      $db->query("ALTER TABLE `cart_items` MODIFY `variant_key` VARCHAR(64) NOT NULL DEFAULT ''");
    }
    // handige indexen
    if (!index_exists($db,'cart_items','idx_cart')) {
      $db->query("CREATE INDEX `idx_cart` ON `cart_items`(`cart_id`)");
    }
  }
}
ensure_cart_tables($mysqli_medewerkers);

/* -- Helpers -- */
function get_or_create_cart_id(mysqli $db,int $partner_id): int{
  $cart_id=0;
  if($st=$db->prepare("SELECT id FROM carts WHERE partner_id=?")){
    $st->bind_param('i',$partner_id);$st->execute();$st->bind_result($cart_id);$st->fetch();$st->close();
  }
  if(!$cart_id && ($st=$db->prepare("INSERT INTO carts(partner_id) VALUES(?)"))){
    $st->bind_param('i',$partner_id);$st->execute();$cart_id=$st->insert_id;$st->close();
  }
  return (int)$cart_id;
}

/* -- Load cart items voor rechterkolom -- */
$cart_id = get_or_create_cart_id($mysqli_medewerkers,$partner_id);
$cart_items=[];
$cart_total_particulier=0.0;
$cart_total_partner=0.0;
...
")){
  ...
  $st->execute();
  $st->bind_result($id,$ptype,$pid,$name,$price,$qty,$vmeta,$margin);

  while($st->fetch()){
    $qtyInt    = (int)$qty;
    $priceF    = (float)$price;          // consumentenprijs incl. btw
    $isBlocked = in_array((int)$pid,[34,35,36,38,39],true);

    // particulieren-lijn
    $line_particulier = $priceF * $qtyInt;

    // btw-nummer partner
    $btw = isset($_SESSION['btw_nummer']) ? strtoupper($_SESSION['btw_nummer']) : '';

    // marge als markup op kostprijs
    $marginPercent = ($margin !== null) ? (float)$margin : 0.0;
    ...
    if($isBlocked){
      ...
      $priceExVat = $priceF / 1.21;

      $markup = $marginPercent / 100;
      if($markup <= -1){
        // veiligheid: onzinwaarde → geen korting
        $partnerPriceExVat = $priceExVat;
      }else{
        // kostprijs uitrekenen op basis van markup (bv. 181.3%)
      ...
      // NL: excl. btw, BE/overig: incl. 21% btw
      if(str_starts_with($btw,'NL')){
        $unit_partner = $partnerPriceExVat;
      }else{
        $unit_partner = $partnerPriceExVat * 1.21;
      }
    }
    ...
    // totalen
    $cart_total_particulier += $line_particulier;
    $cart_total_partner     += $line_partner;

    // parse variant_meta (JSON of TEXT)
    $vm=null;
    if($vmeta!==null && $vmeta!==''){
      $dec=json_decode($vmeta,true);
      if(json_last_error()===JSON_ERROR_NONE && is_array($dec)){ $vm=$dec; }
    }
    ...
    $cart_items[]=[
      ...
      'price'=>$priceF,
      'qty'=>$qtyInt,
      'line_particulier'=>$line_particulier,
...
html[data-price-mode="partner"] .price-particulier{display:none}
...
<main class="cart-page">
  ...
  <div class="cart-card">
    ...
    <?php else: ?>
      <form method="post" action="/pages/account/orders/cart_actions.php">
        ...
        <table class="cart-table">
          ...
          <tbody>
            <?php foreach($cart_items as $it):
              ...
              $isBlocked = !empty($it['blocked']);

              $unitPart    = number_format($it['price'],2,',','.');
              $unitPartner = number_format($it['line_partner'] / max(1,$it['qty']),2,',','.'); // uit lijnbedrag afleiden
              $linePart    = number_format($it['line_particulier'],2,',','.');
              $linePartner = number_format($it['line_partner'],2,',','.');
              ...
              if(is_array($vm) && (!empty($vm['color']) || (!empty($vm['options']) && is_array($vm['options'])))){
                ...
                if(!empty($vm['options']) && is_array($vm['options'])){
                  $chips='';
                  foreach($vm['options'] as $op){ $chips.='<span class="variant-chip">'.h($op).'</span>'; }
                  $parts[]=$chips;
                }
            ...
            ?>
              <tr>
                ...
                <td>
                  ...
                  <?php else: ?>
                    <sup>€</sup>
                    <span class="price-particulier"><?= $unitPart ?></span>
                    <span class="price-partner"><?= $unitPartner ?></span>
                  <?php endif; ?>
                </td>
                <td><input class="qty-input" type="number" min="1" name="qty[<?= (int)$it['id'] ?>]" value="<?= $qty ?>"></td>
                <td>
                  <?php if($isBlocked): ?>
                    <sup>€</sup><?= $linePart ?>
                  <?php else: ?>
                    <sup>€</sup>
                    <span class="price-particulier"><?= $linePart ?></span>
                    <span class="price-partner"><?= $linePartner ?></span>
                  <?php endif; ?>
                </td>
                <td>
                  <button class="btn btn-danger" name="remove_id" value="<?= (int)$it['id'] ?>" title="Verwijderen">🗑</button>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        ...
        <div class="cart-summary">
          <div class="total">
            Totaal:
            <sup>€</sup>
            <span class="price-particulier"><?= number_format($cart_total_particulier,2,',','.') ?></span>
            <span class="price-partner"><?= number_format($cart_total_partner,2,',','.') ?></span>
          </div>
          ...
          <a class="btn btn-primary" href="/pages/account/orders/cart_to_order.php?csrf=<?= urlencode($csrf) ?>">Afrekenen</a>

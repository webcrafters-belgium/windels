...
if($is_partner && empty($_SESSION['csrf'])){ $_SESSION['csrf']=bin2hex(random_bytes(32)); }
$csrf = $_SESSION['csrf'] ?? '';
...
$r=$mysqli_medewerkers->query($sql);
...
if(!empty($items)){
  ...
  if($skus){
    ...
    $in="'".implode("','",$esc)."'";
    $q="SELECT sku, small_desc, large_desc, lenghte, width, breedte, wight FROM product_webshop WHERE sku IN ($in)";
    if($rs=$mysqli_medewerkers->query($q)){
      $bySku=[];
      while($row=$rs->fetch_assoc()){ $bySku[$row['sku']]=$row; }
      $rs->free();
      foreach($items as &$it){
        $sku=$it['sku']??'';
...
<?php if($productId&&isset($items[0])): $p=$items[0];
  ...
  $name=h($p['name']);
  $small_desc = nl2br(h($p['small_desc'] ?? ''));
  $large_desc = nl2br(h($p['large_desc'] ?? ''));
  ...
  $unit_price_raw = number_format((float)$p['price'],2,'.',''); // hidden veld
  ...
  <article class="detail">
    ...
    <div class="detail-info">
      ...
      <div class="detail-meta">
        ...
        <span class="price"><sup>€</sup><?=euro($p['price'])?></span>
      </div>
      ...
      <div class="detail-cta">
        <?php if($is_partner): ?>
          <form method="post" action="/pages/account/orders/cart_actions.php" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
                ...
                <?php foreach($variants as $vx=>$var):
                  ...
                  if($type==='color'): ?>
                    <label class="variant-row">
                      ...
                      <input type="color" id="colorInput" name="variant[color]" value="#000000" data-require-change="1" data-default="#000000">   <!-- standaardkleur om tegen te vergelijken -->
                    </label>
            ...
            <input type="number" id="qtyInput" name="qty" value="1" min="1" class="qty-input" style="width:72px;padding:6px;border:1px solid #c9c9c9;border-radius:6px" readonly>
            <button class="btn" type="submit" style="background:#006c4d;color:#fff;border-radius:10px;padding:.8rem 1.2rem;border:0;cursor:pointer">In winkelwagen</button>
    ...
    <article class="card">
      ...
      <div class="card-body">
        ...
        <div class="card-line">
          ...
          <span class="price"><sup>€</sup><?=euro($it['price'])?></span>
        </div>
